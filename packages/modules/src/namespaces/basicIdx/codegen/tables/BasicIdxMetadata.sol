...
library BasicIdxMetadata {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "basicIdx", name: "BasicIdxMetadata", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462626173696349647800000000000042617369634964784d65746164617461);
  ...
  function get(
    ResourceId sourceTableId,
    bytes32 indexesHash
  ) internal view returns (BasicIdxMetadataData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = ResourceId.unwrap(sourceTableId);
    _keyTuple[1] = indexesHash;
   ...
   */
  function set(ResourceId sourceTableId, bytes32 indexesHash, BasicIdxMetadataData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(ResourceId sourceTableId, bytes32 indexesHash, BasicIdxMetadataData memory _table) internal {
    bytes memory _staticData = encodeStatic(
      _table.has,
      _table.hookAddress,
      _table.keyIndexes,
  ...
  function set(
    ...
    ResourceId sourceTableId,
    bytes32 indexesHash,
    BasicIdxMetadataData memory _table
  ) internal {
    bytes memory _staticData = encodeStatic(
      _table.has,
      _table.hookAddress,
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (BasicIdxMetadataData memory _table) {
    (
      _table.has,
      _table.hookAddress,
      _table.keyIndexes,
