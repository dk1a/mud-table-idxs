...
library Idx_Equipment_TypeLevel {
  ...
  function valuesHash(EquipmentType equipmentType, uint32 level) internal pure returns (bytes32) {
  ...
  function length(EquipmentType equipmentType, uint32 level) internal view returns (uint256) {
    bytes32 _valuesHash = valuesHash(equipmentType, level);
  ...
  function get(EquipmentType equipmentType, uint32 level, uint256 _index) internal view returns (bytes32 entity) {
    bytes32[] memory _keyTuple = getKeyTuple(equipmentType, level, _index);
